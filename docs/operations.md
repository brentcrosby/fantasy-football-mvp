# Operations and Feature Reference

See the [README](../README.md) for the sample demo and concise project overview.
...
The cleanup guard refuses to run against the development `public` schema. Tests never reset or truncate the database; they remove test users, teams, provider fixtures, and the fixture sync record from the test schema. Use a disposable local database or CI service, never a database containing data you need to preserve.
...
The Blueprint selects free plans for initial portfolio testing. Check the provider's current database retention, backup, and pricing terms before treating the deployment as durable.
...
1. Run typecheck, build, database tests, and desktop/mobile browser checks. Review the PR and wait for CI.
2. Merge the approved change. Check the deployed service's auto-deploy setting; do not assume a merge deploys it. Use **Manual Deploy > Deploy latest commit** when necessary.
3. Confirm the deployment's commit matches the intended main revision and `/health` succeeds.
4. Open `/demo` and verify navigation and text export, then check sign-in and a saved team using your own account.
5. Preserve dashboard environment overrides and secrets. Never paste API keys or database connection strings into issues, screenshots, or commits.

The Render dashboard may contain service settings that differ from the Blueprint. Review those before reapplying it. Backups and a restore drill are prerequisites for treating saved teams as durable production data.

## Remaining Production Work

- Verified email ownership and account recovery. Password recovery is not currently implemented.
- A durable, shared AI quota and spending control; the current per-account counter is in memory and resets on restart.
- Backups, recovery testing, and confirmation of hosting retention limits.
- Authenticated end-to-end browser coverage beyond the public demo; API ownership and persistence checks already exist.
- Continued season-boundary and provider-freshness validation.
- Prospective model-versus-provider evaluation before changing the recommendation ranking.

Portfolio readiness means the work is demonstrable, tested, and accurately documented. It is not a claim of production hardening or audited security.

## Dependency Review (2026-09-09)

The CSV parser was updated to 7.0.2 to address the prototype-replacement advisory reported for the previous version. The provider-fixture tests pass with the update.

`npm audit --omit=dev` still reports three high-severity entries along one dependency chain: Prisma -> `@prisma/config` -> `deepmerge-ts` ([advisory](https://github.com/advisories/GHSA-ggr8-5vv4-36mx)). The issue concerns recursive object graphs during merging. In the installed code, this is loaded by Prisma's configuration tooling; this pass did not find an application request path passing user-controlled object graphs into that merger. This is an exposure assessment, not a proof of safety.

The audit's suggested remediation downgrades Prisma outside the declared version range. That was not applied blindly. Track an upstream-compatible fix and verify migrations, client generation, and database tests before changing the ORM dependency chain. Do not describe the dependency audit as clean.
